...
export default function LiveGuidedRoom() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-white/5 dark:backdrop-blur-md flex flex-col p-4 md:p-8 select-none">

            {/* Header */}
            <header className="flex flex-wrap justify-between items-center bg-white dark:bg-white/5 dark:backdrop-blur-md p-4 rounded-xl border dark:border-slate-800 shadow-sm mb-6 gap-4">
                <div className="flex items-center gap-4">
                    <Button variant="ghost" onClick={() => router.push(`/professor/history/${quiz.id}`)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 px-2">
                        <ArrowLeft className="h-5 w-5" />
                    </Button>
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">{quiz.title} <Badge className="ml-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50">LIVE MENTI-STYLE</Badge></h1>
                        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Join Code: <strong className="text-blue-600 dark:text-blue-400">{quiz.code}</strong></p>
                    </div>
                </div>

                <div className="flex items-center gap-4">
                    <div className="text-center">
                        <span className="text-xs text-gray-500 dark:text-gray-400 font-semibold uppercase block">Students</span>
                        <span className="text-xl font-bold text-blue-700 dark:text-blue-400 flex items-center justify-center gap-1"><Users className="h-5 w-5" /> {quiz.attempts.length}</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6 h-full">
                    ...
                    <Card className="bg-blue-600 text-white shadow-lg overflow-hidden border-none shrink-0">
                        <div className="p-6 md:p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
                            ...
                            <div className="bg-white p-3 rounded-xl shadow-inner dark:bg-white">
                    ...
                    {/* Main Stage Content */}
                    <Card className="flex-1 shadow-md border-t-8 border-t-blue-600 flex flex-col overflow-hidden dark:bg-white/5 dark:backdrop-blur-md dark:border-x-slate-800 dark:border-b-slate-800">
                        <CardContent className="p-6 md:p-10 flex flex-col justify-center flex-1 text-center relative min-h-[400px]">

                            {/* PHASE 1: Question */}
                            {phase === "question" && currentQ && (
                                <div className="space-y-6 animate-in fade-in duration-300">
                                    <Badge variant="outline" className="px-4 py-1.5 text-base border-blue-200 dark:border-blue-900/50 text-blue-800 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/20">
                                        Question {activeIndex + 1} of {quiz.questions.length}
                                    </Badge>

                                    <h2 className="text-3xl md:text-5xl font-medium text-gray-900 dark:text-gray-100 leading-tight whitespace-pre-wrap">
                                        {currentQ.text}
                                    </h2>

                                    {currentQ.imageUrl && (
                                        <img src={currentQ.imageUrl} alt="Reference" className="mx-auto mt-8 max-h-64 object-contain rounded-xl border-2 dark:border-slate-700 shadow-sm bg-gray-50 dark:bg-white/5 p-2" />
                                    )}
                                    ...
                                    {currentQ.options && currentQ.options.length > 0 && (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6 max-w-2xl mx-auto text-left">
                                            {currentQ.options.map((opt: any, idx: number) => (
                                                <div key={opt.id || idx} className="p-4 rounded-xl border-2 border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-white/5 text-lg font-medium text-gray-800 dark:text-gray-200">
                                                    <span className="text-blue-600 dark:text-blue-400 font-bold mr-2">{String.fromCharCode(65 + idx)}.</span> {opt.text}
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    {/* Timer */}
                                    <div className={`absolute bottom-6 right-6 flex items-center gap-3 px-6 py-3 rounded-full font-mono text-2xl font-bold border-2 shadow-sm transition-all ${timeLeft <= 5 ? 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border-red-200 dark:border-red-900/50 animate-pulse scale-110' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-900/50'
                                        }`}>
                            ...
                            {phase === "stats" && questionStats && (
                                <div className="space-y-6 animate-in fade-in zoom-in duration-500">
                                    <BarChart3 className="h-16 w-16 text-purple-600 dark:text-purple-500 mx-auto" />
                                    <h3 className="text-3xl font-black text-gray-800 dark:text-gray-100">Question Statistics</h3>
                                    <p className="text-gray-500 dark:text-gray-400">{questionStats.totalResponses} student(s) answered</p>
                                        ...
                                        {questionStats.stats?.map((stat: any, idx: number) => {
                                            ...
                                            return (
                                                <div key={stat.id} className="flex items-center gap-3">
                                                    <span className="text-lg font-bold w-8 text-gray-600 dark:text-gray-400">{String.fromCharCode(65 + idx)}</span>
                                                    <div className="flex-1 bg-gray-100 dark:bg-white/5 rounded-full h-10 relative overflow-hidden">
                                                        <div
                                                    ...
                                                    {stat.isCorrect && <CheckCircle2 className="h-5 w-5 text-green-600 dark:text-green-500" />}
                                                </div>
                                            );
                                        })}
                                    </div>
                                    ...
                                    {/* Correct Answer */}
                                    <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-900/50 rounded-xl inline-block">
                            ...
                            {phase === "leaderboard" && (
                                <div className="space-y-6 animate-in fade-in duration-500">
                                    ...
                                    <h3 className="text-3xl font-black text-gray-800 dark:text-gray-100">🏆 Leaderboard</h3>
                                    ...
                                    <div className="flex items-end justify-center gap-4 h-48 max-w-md mx-auto mt-4">
                                        ...
                                        <div className={`flex flex-col items-center transition-all duration-700 ease-out ${leaderboardAnimStage >= 2 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                                            ...
                                            <p className="text-sm font-bold text-gray-700 dark:text-gray-300 truncate max-w-[100px]">{sortedAttempts[1]?.student?.name || "—"}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{sortedAttempts[1]?.score ?? "—"} pts</p>
                                            <div className="w-20 bg-gray-300 dark:bg-slate-600 rounded-t-lg mt-2" style={{ height: '100px' }}>
                                        ...
                                        <div className={`flex flex-col items-center transition-all duration-700 ease-out ${leaderboardAnimStage >= 3 ? 'opacity-100 translate-y-0 scale-110' : 'opacity-0 translate-y-10'}`}>
                                            ...
                                            <p className="text-sm font-bold text-gray-900 dark:text-gray-100 truncate max-w-[100px]">{sortedAttempts[0]?.student?.name || "—"}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{sortedAttempts[0]?.score ?? "—"} pts</p>
                                            <div className="w-24 bg-yellow-400 dark:bg-yellow-500 rounded-t-lg mt-2" style={{ height: '140px' }}>
                                        ...
                                        <div className={`flex flex-col items-center transition-all duration-700 ease-out ${leaderboardAnimStage >= 1 ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                                            ...
                                            <p className="text-sm font-bold text-gray-700 dark:text-gray-300 truncate max-w-[100px]">{sortedAttempts[2]?.student?.name || "—"}</p>
                                            <p className="text-xs text-gray-500 dark:text-gray-400">{sortedAttempts[2]?.score ?? "—"} pts</p>
                ...
                <Card className="shadow-md h-full flex flex-col dark:bg-white/5 dark:backdrop-blur-md dark:border-slate-800">
                    <div className="p-4 border-b dark:border-slate-800 bg-gray-50 dark:bg-white/5/50 rounded-t-xl">
                        <h3 className="text-lg font-bold text-gray-800 dark:text-gray-100 flex items-center gap-2"><BarChart3 className="h-5 w-5 text-indigo-600 dark:text-indigo-400" /> Live Rankings</h3>
                    </div>
                    <div className="p-4 flex-1 overflow-y-auto space-y-3">
                        {sortedAttempts.length === 0 ? (
                            ...
                            sortedAttempts.map((attempt: any, idx: number) => (
                                <div key={attempt.id} className={`flex items-center gap-3 bg-white dark:bg-white/5 dark:backdrop-blur-md p-3 rounded-xl border dark:border-slate-800 hover:shadow-sm transition-all ${phase === "leaderboard" && leaderboardAnimStage >= 4 ? 'animate-in fade-in slide-in-from-right duration-300' : ''
                                    }`} style={{ animationDelay: `${idx * 80}ms` }}>
                                    <div className={`h-8 w-8 rounded-full flex items-center justify-center font-bold text-sm ${idx === 0 ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' :
                                            idx === 1 ? 'bg-gray-200 dark:bg-slate-700 text-gray-700 dark:text-gray-300' :
                                                idx === 2 ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'
                                        }`}>
                                        #{idx + 1}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-semibold text-gray-900 dark:text-gray-200 truncate">{attempt.student?.name || "Student"}</p>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{attempt.student?.email}</p>
                                    </div>
                                    <div className="font-bold text-lg text-indigo-600 dark:text-indigo-400">{attempt.score}</div>
                                </div>
                            ))
                        )}
                    </div>
